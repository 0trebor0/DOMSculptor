...
let budgetBytes = 10 * 1024;

console.log(`dist/domsculptor.min.js: ${gzipBytes} bytes minified and gzipped (budget: ${budgetBytes})`);
assert.ok(gzipBytes <= budgetBytes, `Bundle exceeds the 10 KB gzip budget by ${gzipBytes - budgetBytes} bytes`);
...
assert.ok(
    ...
    `Browser ESM bundle exceeds the 10 KB gzip budget by ${esmGzipBytes - budgetBytes} bytes`
);
